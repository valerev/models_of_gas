...
def x_reflection(beta):
    for i_vx in range(n_vx):
        for i_vy in range(n_vy):
            v = get_v(i_vx, i_vy)
            if v[0] < 0:
                data[0, :, i_vx, i_vy] += beta * prev_data[0, :, n_vx - i_vx - 1, i_vy]
            else:
                data[n_x - 1, :, i_vx, i_vy] += beta * prev_data[n_x - 1, :, n_vx - i_vx - 1, i_vy]
                data[wall_x - 1, wall_y, i_vx, i_vy] += beta * prev_data[wall_x - 1, wall_y, n_vx - i_vx - 1, i_vy]


def y_reflection(beta):
    for i_vx in range(n_vx):
        for i_vy in range(n_vy):
            v = get_v(i_vx, i_vy)
            if v[1] < 0:
                data[:, 0, i_vx, i_vy] += beta * prev_data[:, 0, i_vx, n_vy - i_vy - 1]
                data[wall_x:, wall_y + 1, i_vx, i_vy] += beta * prev_data[wall_x:, wall_y + 1, i_vx, n_vy - i_vy - 1]
            else:
                data[:, n_y - 1, i_vx, i_vy] += beta * prev_data[:, n_y - 1, i_vx, n_vy - i_vy - 1]
                data[wall_x:, wall_y - 1, i_vx, i_vy] += beta * prev_data[wall_x:, wall_y - 1, i_vx, n_vy - i_vy - 1]
