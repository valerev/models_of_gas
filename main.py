...
def count_x_collisions_with_walls(time):
    ...
    for y in range(0, N): # x = 0, v_x < 0
        for v_x in range(number_of_velocities // 2):
            a = abs((v_x - number_of_velocities // 2) * dt / dx)
            for v_y in range(number_of_velocities):
                f[time][0][y][v_x][v_y] = f[pre_time][0][y][number_of_velocities - 1 - v_x][v_y]
                # f[time][0][y][v_x][v_y] = f[pre_time][0][y][v_x][v_y] + \
                #                           a * f[pre_time][1][y][v_x][v_y] - a * f[pre_time][0][y][v_x][v_y]
    ...
    for y in range(0, N): # x = 0, v_x > 0
        for v_x in range(number_of_velocities // 2 + 1, number_of_velocities):
            ...
            for v_y in range(number_of_velocities):
                # f[time][0][y][v_x][v_y] = f[pre_time][0][y][number_of_velocities - 1 - v_x][v_y]
                # f[time][0][y][v_x][v_y] = a * f[pre_time][0][y][number_of_velocities - 1 - v_x][v_y] - \
                #                           (a - 1) * f[pre_time][0][y][v_x][v_y]
                f[time][0][y][v_x][v_y] = f[pre_time][0][y][v_x][v_y] + \
                                              a * f[pre_time][1][y][v_x][v_y] - a * f[pre_time][0][y][v_x][v_y]

    for y in range(0, N):  # x = N-1, v_x > 0
        for v_x in range(number_of_velocities // 2 + 1, number_of_velocities):
            a = abs((v_x - number_of_velocities // 2) * dt / dx)
            for v_y in range(number_of_velocities):
                f[time][N - 1][y][v_x][v_y] = f[pre_time][N - 1][y][number_of_velocities - 1 - v_x][v_y]
                # f[time][N-1][y][v_x][v_y] = f[pre_time][N-1][y][v_x][v_y] + \
                #                             a * f[pre_time][N-1][y][v_x][v_y] - a * f[pre_time][N-2][y][v_x][v_y]
    ...
    for y in range(0, N): # x = N - 1, v_x < 0
        for v_x in range(number_of_velocities // 2):
            ...
            for v_y in range(number_of_velocities):
                # f[time][N - 1][y][v_x][v_y] = f[pre_time][N - 1][y][number_of_velocities - 1 - v_x][v_y]
                # f[time][N-1][y][v_x][v_y] = a * f[pre_time][N-1][y][number_of_velocities - 1 - v_x][v_y] - \
                #                             (a - 1) * f[pre_time][N-1][y][v_x][v_y]
                f[time][N-1][y][v_x][v_y] = f[pre_time][N-1][y][v_x][v_y] + \
                                            a * f[pre_time][N-2][y][v_x][v_y] - a * f[pre_time][N-1][y][v_x][v_y]


def count_y_collisions_with_walls(time):
    pre_time = (time - 1)%2

    for x in range(0, N): # y = 0, v_y < 0
    ...
    for x in range(0, N): # y = 0, v_y > 0
        for v_y in range(number_of_velocities // 2 + 1, number_of_velocities):
            ...
            for v_x in range(number_of_velocities):
                # f[time][x][0][v_x][v_y] = f[pre_time][x][0][v_x][number_of_velocities - 1 - v_y]
                # f[time][x][0][v_x][v_y] = a * f[pre_time][x][0][v_x][number_of_velocities - 1 - v_y] - \
                #                           (a - 1) * f[pre_time][x][0][v_x][v_y]
                f[time][x][0][v_x][v_y] = f[pre_time][x][0][v_x][v_y] + \
                                          a * f[pre_time][x][1][v_x][v_y] - a * f[pre_time][x][0][v_x][v_y]

    for x in range(0, N): # y = N-1, v_y > 0
        for v_y in range(number_of_velocities // 2 + 1, number_of_velocities):
            a = abs((v_y - number_of_velocities // 2) * dt / dy)
            for v_x in range(number_of_velocities):
    ...
    for x in range(0, N): # y = N-1, v_y < 0
        for v_y in range(number_of_velocities // 2):
            a = abs((v_y - number_of_velocities // 2) * dt / dy)
            for v_x in range(number_of_velocities):
                # f[time][x][N - 1][v_x][v_y] = a * f[pre_time][x][N-1][v_x][number_of_velocities - 1 - v_y] - \
                #                               (a - 1) * f[pre_time][x][N - 1][v_x][v_y]
                f[time][x][N - 1][v_x][v_y] = f[pre_time][x][N - 1][v_x][v_y] + \
                                              a * f[pre_time][x][N - 2][v_x][v_y] - a * f[pre_time][x][N - 1][v_x][v_y]
...
def count_two_steps(n_step):
    concentration = np.zeros(N * N).reshape(N, N)
    for x in range(1, N - 1):  # Making step using only x
        for y in range(0, N):
            for v_x in range(number_of_velocities):
                for v_y in range(number_of_velocities):
                    make_x_step(x, y, v_x, v_y)
